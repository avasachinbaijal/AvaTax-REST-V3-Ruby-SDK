...
```shell
gem build Avalara.SDK.gemspec
```

Then either install the gem locally:

```shell
gem install ./Avalara.SDK-2.3.0.gem
```

(for development, run `gem install --dev ./Avalara.SDK-2.3.0.gem` to install the development dependencies)

or publish the gem to a gem hosting service, e.g. [RubyGems](https://rubygems.org/).

Finally add this to the Gemfile:

    gem 'Avalara.SDK', '~> 2.3.0'

### Install from Git

If the Ruby gem is hosted at a git repository: https://github.com/GIT_USER_ID/GIT_REPO_ID, then add the following in the Gemfile:

    gem 'Avalara.SDK', :git => 'https://github.com/GIT_USER_ID/GIT_REPO_ID.git'
