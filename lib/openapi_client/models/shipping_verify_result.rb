  ...
  class ShippingVerifyResult
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ShippingVerifyResult` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ShippingVerifyResult`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'warning_codes')
        if (value = attributes[:'warning_codes']).is_a?(Array)
          self.warning_codes = value
        end
      end
    ...
    def hash
      [compliant, message, success_messages, failure_messages, failure_codes, warning_codes, lines].hash
    end
